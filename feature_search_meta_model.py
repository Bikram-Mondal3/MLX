...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Feature Search and Stacked Meta-Model for Solar Radiation")
    ...
    parser.add_argument("--submission", default="stacked_submission.csv", help="Output submission CSV path.")
    parser.add_argument("--metrics", default="stacked_cv_metrics.json", help="Output CV metrics JSON path.")
...
def build_stacked_model(random_state: int) -> Pipeline:
    # 1. Feature Selector (using Extra Trees for robust importance)
    feature_selector = SelectFromModel(
        ExtraTreesRegressor(n_estimators=100, random_state=random_state, n_jobs=-1),
        threshold="median", # Keep top 50% features, can be tuned
    )

    # 2. Base Estimators for the Stacked Meta-Model
    ...
    # 3. Stacked Regressor with a Ridge Regression Meta-Model
    ...
    return Pipeline(
        [
            ...
            ("stacked", stacked_regressor),
        ]
    )


def cross_validate(model: Pipeline, X: pd.DataFrame, y: pd.Series, groups: pd.Series, folds: int) -> dict:
    ...
    for fold, (train_idx, valid_idx) in enumerate(splitter.split(X, y, groups=groups), start=1):
        ...
        print(f"Fold {fold} - RMSE: {fold_rmse:.4f}, MAE: {fold_mae:.4f}, Features Selected: {selected_features}")
...
def main() -> None:
    ...
    print("Building stacked meta-model pipeline...")
    model = build_stacked_model(random_state=args.random_state)
    ...
    # Ensure no negative radiation predictions
    ...
    try:
        ...
        print(f"Selected {len(selected_cols)} features for final model: {selected_cols}")
    except AttributeError:
        pass
